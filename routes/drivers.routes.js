  ...
  async (req, res) => {
    try {
      const {
        ...
        birthDate, nationalId, nationalIdExpire, province, areaAddress,
      } = req.body;

      if (![first, father, grandfather, fourth, last, phone, licenseEnd, nationalIdExpire].every(Boolean)) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        ![first, father, grandfather, fourth, last, phone, licenseEnd, nationalIdExpire].every(Boolean)
      ) {
        return res.status(400).json({ error: "جميع الحقول الأساسية مطلوبة" });
      }
      ...
      driver.nationalIdExpire = nationalIdExpire;
