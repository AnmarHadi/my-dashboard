...
const dto = (v, req) => ({
  ...
  // تم التعديل هنا: ownerAddress وليس address
...
router.post(
  ...
  async (req, res) => {
    try {
      // لاحظ ownerAddress
      const { number, province, contractor, annualEnd, checkupEnd, wheelType, ownerName, ownerAddress } = req.body;
      ...
      const vehicle = new Vehicle({
        ...
        wheelType, ownerName, ownerAddress,
      });
      await vehicle.save();
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      vehicle.ownerAddress = ownerAddress; // <-- هنا كان الفرق
